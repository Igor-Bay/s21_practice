...
void init(char*** matrix, int height, int width, int* gen, int* speed);
...
void init(char*** matrix, int height, int width, int* gen, int* speed) {
    *gen = 0;
    ...
    addFigure(*matrix);   // функция добавляет фигуру через консоль и из файла. Работаю над вводом фигуры из файла
...
// данную функцию еще не дописал полностью
char getKey() {
    char key;
    while (1) {
        if (scanf("%c", &key) == -1) {
            printf("Form accepted");
            freopen("/dev/tty", "r", stdin);
        }
        if ((key >= '1') && (key <= '5'))
            break;
